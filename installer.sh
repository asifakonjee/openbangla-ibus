...
#### OpenBangla Keyboard for ibus Installation Script by ####
...
# log directory
log_dir="$present_dir/.openbangla-ibus"
...
if [[ -d "$cache_dir/openbangla-ibus" ]]; then
    printf "${note} - Directory '${orange}openbangla-ibus${end}' was located in the '${cache_dir}' directory. Removing it.\n" && sleep 1
    sudo rm -r "$cache_dir/openbangla-ibus"
fi

git clone --recursive https://github.com/asifakonjee/openbangla-ibus.git "$cache_dir/openbangla-ibus" 2>&1 | tee -a "$log" || { printf "${error} - Sorry, could not clone openbangla-ibus repository\n" 2>&1 | tee -a >(sed 's/\x1B\[[0-9;]*[JKmsu]//g' >> "$log"); exit 1; }

# Move into the cloned directory
cd "$cache_dir/openbangla-ibus" || { printf "${error} - Unable to change directory\n" 2>&1 | tee -a >(sed 's/\x1B\[[0-9;]*[JKmsu]//g' >> "$log"); exit 1; }
